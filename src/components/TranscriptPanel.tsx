    ...
    <section className="workspace-card">
      <div className="workspace-toolbar">
        ...
        <div className="toolbar-actions">
          ...
          <button className="toolbar-button" onClick={onToggleOriginal} title={showOriginal ? "隐藏原文" : "显示原文"}>
            {showOriginal ? <Eye size={16} /> : <EyeOff size={16} />}原文
          </button>
          <button className="toolbar-button" onClick={onOpenCaption}>
            <PanelTop size={16} />打开浮窗
          </button>
          <button className="icon-button subtle" onClick={onClear} aria-label="清空记录" title="清空记录">
            <Trash2 size={16} />
          </button>
        </div>
      </div>
      <div className="transcript-list">
        {filtered.length === 0 ? (
          <div className="empty-state">
            <div className="empty-icon"><ListFilter size={21} /></div>
            <h2>{query ? "没有找到匹配字幕" : "准备好开始了吗？"}</h2>
            <p>{query ? "换一个关键词试试。" : "点击“开始翻译”，字幕会出现在这里。浮窗可以叠加在视频上方。"}</p>
          </div>
        ) : filtered.map((entry) => (
          <article className={"transcript-row " + (entry.is_final ? "final" : "partial")} key={entry.id}>
      ...
      <footer className="workspace-footer">
        <div className="footer-status"><span className={"audio-bars " + (session.state === "listening" ? "active" : "")}><i /><i /><i /><i /></span>{session.state === "listening" ? `正在接收${AUDIO_SOURCE_LABEL[audioSource]}` : `${AUDIO_SOURCE_LABEL[audioSource]}未启动`}</div>
        <div className="footer-actions">
          <button className="text-button" onClick={() => void onCopy(filtered)} disabled={filtered.length === 0}><Copy size={15} />复制字幕</button>
          <button className="text-button" onClick={() => void onExport(filtered)} disabled={filtered.length === 0}><Download size={15} />导出</button>
        </div>
      </footer>
    </section>
  );
